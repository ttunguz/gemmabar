...
struct MLXServer: AsyncParsableCommand {
    ...
    @Option(name: .long, help: "Path to a system prompt injected into every chat request")
    ...
    mutating func run() async throws {
        ...
        if let systemPromptFile {
            do {
                let prompt = try String(contentsOfFile: systemPromptFile, encoding: .utf8)
                    .trimmingCharacters(in: .whitespacesAndNewlines)
                defaultSystemPrompt = prompt.isEmpty ? nil : prompt
            } catch {
                print("[SwiftLM] ⚠️  Could not read --system-prompt-file \(systemPromptFile): \(error.localizedDescription)")
...
func handleChatCompletion(
    ...
    // Server-level prompt, used by wrappers such as GemmaBar to apply consistent
    // dictation behavior without requiring every OpenAI-compatible client to send it.
    if let defaultSystemPrompt = config.defaultSystemPrompt, !defaultSystemPrompt.isEmpty {
        if !chatMessages.isEmpty && chatMessages[0].role == .system {
            chatMessages[0].content = defaultSystemPrompt + "\n\n" + chatMessages[0].content
        } else {
            chatMessages.insert(.system(defaultSystemPrompt), at: 0)
        }
